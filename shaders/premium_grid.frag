...
void main() {
    ...
    float hazeIntens = lightMode ? 0.25 + (uVelocity * 0.2) : 0.4 + (uVelocity * 0.3);
    ...
    // In light mode, use a slightly deeper blue for better visibility
    vec3 glowColor = lightMode ? vec3(0.0, 0.4, 0.8) : vec3(0.0, 0.5, 1.0);
    color += glowColor * haze * pulse;
