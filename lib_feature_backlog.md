...
|---|---|---|---|---|---|
| `wifi_scan` | Wi-Fi ağlarını kanal, RSSI, security, WPS/PMF, width, standard bilgileriyle tarıyor. 6GHz **destekli**: `WifiBand.ghz6` enum (`wifi_band.dart:2`), `bandFromFrequency` 5925-7200 MHz aralığını tanıyor, `channel_rating_engine.dart:27`'de UNII-5..8 kanalları + UI'da "6 GHz" etiketleri. DFS handling 5GHz için var: `channel_rating_engine.dart:142-148` `isDfsChannel` + score -0.5 penalty. Wi-Fi 7 (BE) `wifi_network.dart:32`'de tanımlı. | Eksik: ülke/regülasyon profili (TR/EU/US), DFS handling 6GHz UNII-5..8'e genişletilmemiş, "Wi-Fi 6E" özel etiketi yok (sadece "6 GHz"). | İleri router/AP'lerde yetersiz açıklama; ülke uyumsuzluğu uyarısı yok. | Country/regulatory profile (ETSI/FCC/ARIB), DFS'i 6GHz'e genişlet, Wi-Fi 6E ayrımı için band+standart kombinasyonuna özel etiket. | P2 |
...
| `wifi_scan` | Multi-pass snapshot + karşılaştırma (`scan_comparison_service.dart:18`) var. Android throttling cache banner mevcut (`wifi_scan_page.dart:430-464`): "Showing cached results — Android limits scan frequency. Wait ~30 s and refresh for live data." | Banner statik metin; canlı geri sayım sayacı (countdown timer) yok. | Kullanıcı kaç saniye sonra yeniden taranabilir olduğunu görmüyor. | Statik "~30 s" yerine kalan saniyeyi gösteren live countdown ekle. | P2 |
...
| `network_scan` | Açık port ve servis banner okuyor. `host_scan_result.dart:18` `isGateway` flag'i var; `TrustedNetworkProfile.gateway` field'ı (`trusted_network_profile.dart:36`) tanımlı ama baseline diff/uyarı tüketicisi yok. | Rogue DHCP / gateway değişimi tespit edilip kullanıcıya raporlanmıyor. | Sahte gateway veya yanlış DHCP ciddi MITM riski yaratır. | Gateway MAC baseline'ı `TrustedNetworkProfile`'a kaydet (field hazır), drift detector'a gateway karşılaştırma ekle, kritik bildirim olarak çıkar. DHCP sunucu gözlemi de eklenmeli. | P0b |
...
| `performance` | Loaded latency ölçülüyor. Bufferbloat A-F skorlama Waveform standardında **tam implement edilmiş** (`performance_page.dart:573-665` `_BufferbloatGradeCard`): Excellent → Critical (6 kademe), her kademe için "Enable QoS", "Upgrade firmware" gibi insan-okunur açıklama. | — (zaten yapılmış, takip için tutuluyor) | Kullanıcı oyun/video sorununu net görüyor. | İsteğe bağlı: skor tarihçesi (zaman içinde bufferbloat eğilimi) ve gateway ping korelasyonu. | P2 |
...
**P2 ve sonrası:** Skor breakdown detaylandırma, AI explainability, model card, çoklu kat heatmap UI, IPv6, ülke/regülasyon profili (DFS 6GHz + Wi-Fi 6E etiketi), bufferbloat tarihçesi.
...
- **Arka plan izleme (P0b)** → rogue DHCP / gateway baseline tamam olmadan etkili çalışmaz. `TrustedNetworkProfile.gateway` field'ı (`trusted_network_profile.dart:36`) zaten hazır; eksik olan baseline persistence ve drift detector'da gateway karşılaştırması.
...
- **Bufferbloat A-F skoru:** Backlog "skor eklenmeli" diyordu (P1); `_BufferbloatGradeCard` (`performance_page.dart:573-665`) Waveform A-F standardını tam **implement etmiş** (Excellent → Critical, 6 kademe, insan-okunur açıklamalar). README.md:44 hâlâ eski; backlog README'den türetilmiş olduğu için yanlış. P1 → P2'ye düşürüldü.
- **6GHz / Wi-Fi 6E / DFS:** Backlog "eksik" diyordu (P1); 6GHz **band desteği var** (`wifi_band.dart:2` enum, `bandFromFrequency` 5925-7200), `channel_rating_engine.dart:27` UNII-5..8 kanal listesi, UI'da "6 GHz" etiketi. 5GHz **DFS handling tam** (`channel_rating_engine.dart:142` `isDfsChannel` + score penalty). Wi-Fi 7 standardı `wifi_network.dart:32`'de tanımlı. Gerçek eksik: ülke/regülasyon profili, DFS'in 6GHz UNII-5..8'e genişletilmesi, "Wi-Fi 6E" özel etiketi. Formülasyon ve öncelik (P1 → P2) düzeltildi.
- **Android throttling feedback:** Backlog "zayıf" diyordu; `wifi_scan_page.dart:430-464` cache banner'ı ("Showing cached results — Android limits scan frequency. Wait ~30 s...") **mevcut**. Eksik olan **canlı countdown timer** — formülasyon ona daraltıldı.
- **README/kod farkı uyarısı:** `wifi_scan/README.md` ve `performance/README.md` güncel değil; bufferbloat A-F, 6GHz desteği, throttling banner README'de "yok" yazıyor ama kodda var. Backlog yazılırken README'ye değil **koda** bakılması gerek.
