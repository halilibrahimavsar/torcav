import 'package:flutter/material.dart';
import 'package:google_fonts/google_fonts.dart';

import '../../../../core/di/injection.dart';
import '../../../../core/extensions/context_extensions.dart';
import '../../../../core/theme/neon_widgets.dart';
import '../../../wifi_scan/domain/entities/wifi_network.dart';
import '../../../wifi_scan/domain/services/scan_session_store.dart';
import '../../domain/entities/dns_test_result.dart';
import '../../domain/entities/vulnerability.dart';
import '../../domain/usecases/dns_leak_test_usecase.dart';
import '../../domain/usecases/security_analyzer.dart';
import '../extensions/vulnerability_extensions.dart';

/// On-demand deep security audit, embedded inside the Security Center.
///
/// Runs the encryption, DNS-leak and network-environment tests that used to
/// live in the standalone Vulnerability Lab. The always-on Security Center
/// covers live posture; this section is the manual, comprehensive sweep.
class SecurityAuditSection extends StatefulWidget {
  const SecurityAuditSection({super.key});

  @override
  State<SecurityAuditSection> createState() => _SecurityAuditSectionState();
}

class _SecurityAuditSectionState extends State<SecurityAuditSection> {
  final SecurityAnalyzer _analyzer = getIt<SecurityAnalyzer>();
  final ScanSessionStore _sessionStore = getIt<ScanSessionStore>();
  final DnsLeakTestUsecase _dnsTest = getIt<DnsLeakTestUsecase>();

  bool _isRunning = false;
  bool _hasRun = false;
  List<_TestSection> _results = [];

  List<WifiNetwork> _getConnectedNetworks() {
    final snapshot = _sessionStore.latest;
    if (snapshot == null) return [];
    return snapshot.networks.map((n) => n.toWifiNetwork()).toList();
  }

  Future<void> _runAllTests() async {
    setState(() {
      _isRunning = true;
      _results = [];
    });

    final networks = _getConnectedNetworks();
    final sections = <_TestSection>[];
    final l10n = context.l10n;

    // ── Test 1: Network Encryption Analysis ──
    final encryptionFindings = <Vulnerability>[];
    for (final net in networks) {
      final assessment = _analyzer.assess(net, localBaseline: networks);
      encryptionFindings.addAll(assessment.findings);
    }
    final seen = <String>{};
    final uniqueFindings =
        encryptionFindings.where((v) {
          if (seen.contains(v.title)) return false;
          seen.add(v.title);
          return true;
        }).toList();

    sections.add(
      _TestSection(
        title: l10n.encryptionAndConfigTitle,
        icon: Icons.lock_outline_rounded,
        findings: uniqueFindings,
      ),
    );

    // ── Test 2: DNS Leak Test ──
    final dnsFindings = <Vulnerability>[];
    final dnsResult = await _dnsTest(null);
    dnsResult.fold(
      (_) => dnsFindings.add(
        Vulnerability(
          title: l10n.dnsTestFailedTitle,
          description: l10n.dnsTestFailedDesc,
          severity: VulnerabilitySeverity.info,
          recommendation: l10n.retryInternetConnection,
        ),
      ),
      (result) {
        if (result.isLeaking) {
          dnsFindings.add(
            Vulnerability(
              title: l10n.dnsLeakDetectedTitle,
              description: l10n.dnsLeakDetectedDesc,
              severity: VulnerabilitySeverity.high,
              recommendation: l10n.dnsLeakRecommendation,
            ),
          );
        }
        if (result.isHijacked) {
          dnsFindings.add(
            Vulnerability(
              title: l10n.dnsHijackingDetectedTitle,
              description: l10n.dnsHijackingDetectedDesc,
              severity: VulnerabilitySeverity.critical,
              recommendation: l10n.dnsHijackingRecommendation,
            ),
          );
        }
        if (result.status == DnsSecurityStatus.warning) {
          dnsFindings.add(
            Vulnerability(
              title: l10n.dnsConfigWarningTitle,
              description: l10n.dnsConfigWarningDesc,
              severity: VulnerabilitySeverity.medium,
              recommendation: l10n.dnsConfigRecommendation,
            ),
          );
        }
      },
    );
    sections.add(
      _TestSection(
        title: l10n.dnsSecurityTitle,
        icon: Icons.dns_rounded,
        findings: dnsFindings,
      ),
    );

    // ── Test 3: Network Environment ──
    final envFindings = <Vulnerability>[];
    final openNets =
        networks.where((n) => n.security == SecurityType.open).length;
    if (openNets > 0) {
      envFindings.add(
        Vulnerability(
          title: l10n.openNetworksNearbyTitle(openNets),
          description: l10n.openNetworksNearbyDesc(openNets),
          severity: VulnerabilitySeverity.info,
          recommendation: l10n.vpnRecommendation,
        ),
      );
    }
    final wpsNets = networks.where((n) => n.hasWps == true).length;
    if (wpsNets > 0) {
      envFindings.add(
        Vulnerability(
          title: l10n.wpsEnabledNearbyTitle(wpsNets),
          description: l10n.wpsEnabledNearbyDesc(wpsNets),
          severity: VulnerabilitySeverity.medium,
          recommendation: l10n.wpsRecommendation,
        ),
      );
    }
    sections.add(
      _TestSection(
        title: l10n.environmentScanTitle,
        icon: Icons.radar_rounded,
        findings: envFindings,
      ),
    );

    if (mounted) {
      setState(() {
        _isRunning = false;
        _hasRun = true;
        _results = sections;
      });
    }
  }

  @override
  Widget build(BuildContext context) {
    final l10n = context.l10n;
    final scheme = Theme.of(context).colorScheme;
    final networks = _getConnectedNetworks();
    final totalFindings = _results.fold<int>(
      0,
      (sum, s) => sum + s.findings.length,
    );

    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        NeonSectionHeader(
          label: l10n.vulnLabTitle,
          icon: Icons.biotech_rounded,
          color: scheme.secondary,
        ),
        const SizedBox(height: 12),

        // ── Run control ──
        NeonCard(
          glowColor: scheme.primary,
          glowIntensity: 0.08,
          padding: const EdgeInsets.all(20),
          child: Column(
            children: [
              Text(
                l10n.vulnLabSubtitle,
                style: GoogleFonts.rajdhani(
                  color: scheme.onSurfaceVariant,
                  fontSize: 14,
                  fontWeight: FontWeight.w600,
                ),
                textAlign: TextAlign.center,
              ),
              const SizedBox(height: 16),
              if (networks.isEmpty)
                Text(
                  l10n.vulnLabNoNetwork,
                  style: GoogleFonts.rajdhani(
                    color: scheme.error,
                    fontSize: 13,
                  ),
                  textAlign: TextAlign.center,
                )
              else
                _isRunning
                    ? Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        SizedBox(
                          width: 16,
                          height: 16,
                          child: CircularProgressIndicator(
                            strokeWidth: 2,
                            color: scheme.primary,
                          ),
                        ),
                        const SizedBox(width: 12),
                        Text(
                          l10n.vulnLabRunning,
                          style: GoogleFonts.orbitron(
                            color: scheme.primary.withValues(alpha: 0.6),
                            fontSize: 11,
                            fontWeight: FontWeight.bold,
                          ),
                        ),
                      ],
                    )
                    : NeonButton(
                      onPressed: _runAllTests,
                      label: l10n.vulnLabRunAll,
                      icon: Icons.play_arrow_rounded,
                      color: scheme.primary,
                    ),
            ],
          ),
        ),

        // ── Summary ──
        if (_hasRun && !_isRunning) ...[
          const SizedBox(height: 16),
          NeonCard(
            glowColor: totalFindings > 0 ? scheme.error : scheme.tertiary,
            glowIntensity: 0.10,
            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14),
            child: Row(
              children: [
                Icon(
                  totalFindings > 0
                      ? Icons.warning_amber_rounded
                      : Icons.verified_rounded,
                  color: totalFindings > 0 ? scheme.error : scheme.tertiary,
                  size: 28,
                ),
                const SizedBox(width: 14),
                Expanded(
                  child: Text(
                    totalFindings > 0
                        ? l10n.vulnLabFoundCount(totalFindings)
                        : l10n.vulnLabAllClear,
                    style: GoogleFonts.orbitron(
                      color:
                          totalFindings > 0 ? scheme.error : scheme.tertiary,
                      fontSize: 12,
                      fontWeight: FontWeight.bold,
                    ),
                  ),
                ),
              ],
            ),
          ),
        ],

        // ── Test sections ──
        ..._results.map(
          (section) => Padding(
            padding: const EdgeInsets.only(top: 20),
            child: _buildTestSection(context, section),
          ),
        ),
      ],
    );
  }

  Widget _buildTestSection(BuildContext context, _TestSection section) {
    final scheme = Theme.of(context).colorScheme;
    final l10n = context.l10n;
    final hasIssues = section.findings.isNotEmpty;

    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        NeonSectionHeader(
          label: section.title,
          icon: section.icon,
          color: hasIssues ? scheme.error : scheme.tertiary,
        ),
        const SizedBox(height: 12),
        if (section.findings.isEmpty)
          NeonCard(
            glowColor: scheme.tertiary,
            glowIntensity: 0.04,
            child: Row(
              children: [
                Icon(
                  Icons.check_circle_outline_rounded,
                  color: scheme.tertiary,
                  size: 20,
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: Text(
                    l10n.noIssuesDetected,
                    style: GoogleFonts.rajdhani(
                      color: scheme.tertiary,
                      fontSize: 14,
                      fontWeight: FontWeight.w600,
                    ),
                  ),
                ),
              ],
            ),
          )
        else
          ...section.findings.map(
            (v) => Padding(
              padding: const EdgeInsets.only(bottom: 8),
              child: _VulnerabilityCard(vulnerability: v),
            ),
          ),
      ],
    );
  }
}

class _TestSection {
  final String title;
  final IconData icon;
  final List<Vulnerability> findings;

  const _TestSection({
    required this.title,
    required this.icon,
    required this.findings,
  });
}

class _VulnerabilityCard extends StatelessWidget {
  final Vulnerability vulnerability;

  const _VulnerabilityCard({required this.vulnerability});

  Color _severityColor(ColorScheme scheme) {
    return switch (vulnerability.severity) {
      VulnerabilitySeverity.critical => scheme.error,
      VulnerabilitySeverity.high => const Color(0xFFFF6B35),
      VulnerabilitySeverity.medium => const Color(0xFFFFB300),
      VulnerabilitySeverity.low => scheme.primary,
      VulnerabilitySeverity.info => scheme.onSurfaceVariant,
    };
  }

  @override
  Widget build(BuildContext context) {
    final scheme = Theme.of(context).colorScheme;
    final color = _severityColor(scheme);

    return NeonCard(
      glowColor: color,
      glowIntensity: 0.06,
      padding: const EdgeInsets.all(14),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(
            children: [
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                decoration: BoxDecoration(
                  color: color.withValues(alpha: 0.15),
                  borderRadius: BorderRadius.circular(4),
                  border: Border.all(color: color.withValues(alpha: 0.4)),
                ),
                child: Text(
                  vulnerability.severity.localized(context),
                  style: GoogleFonts.orbitron(
                    color: color,
                    fontSize: 8,
                    fontWeight: FontWeight.bold,
                  ),
                ),
              ),
              const SizedBox(width: 10),
              Expanded(
                child: Text(
                  vulnerability.localizedTitle(context),
                  style: GoogleFonts.orbitron(
                    color: color,
                    fontSize: 11,
                    fontWeight: FontWeight.bold,
                  ),
                ),
              ),
            ],
          ),
          const SizedBox(height: 10),
          Text(
            vulnerability.localizedDescription(context),
            style: GoogleFonts.rajdhani(
              color: scheme.onSurface.withValues(alpha: 0.8),
              fontSize: 13,
              height: 1.4,
            ),
          ),
          const SizedBox(height: 10),
          Container(
            padding: const EdgeInsets.all(10),
            decoration: BoxDecoration(
              color: scheme.primary.withValues(alpha: 0.05),
              borderRadius: BorderRadius.circular(8),
              border: Border.all(color: scheme.primary.withValues(alpha: 0.15)),
            ),
            child: Row(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Icon(
                  Icons.lightbulb_outline_rounded,
                  color: scheme.primary.withValues(alpha: 0.6),
                  size: 14,
                ),
                const SizedBox(width: 8),
                Expanded(
                  child: Text(
                    vulnerability.localizedRecommendation(context),
                    style: GoogleFonts.rajdhani(
                      color: scheme.onSurfaceVariant,
                      fontSize: 12,
                      height: 1.3,
                    ),
                  ),
                ),
              ],
            ),
          ),
        ],
      ),
    );
  }
}
