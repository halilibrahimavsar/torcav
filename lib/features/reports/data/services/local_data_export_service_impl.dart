...
class LocalDataExportServiceImpl implements LocalDataExportService {
  ...
  Future<String> exportCategory(
    ...
    return switch (format) {
      ...
      ExportFormat.html =>
        _wrapHtml([(category, payload)], anonymize: anonymize),
    };
  }
  ...
  Future<String> exportAll({
    ...
    return switch (format) {
      ...
      ExportFormat.csv => throw const FormatException(
  ...
  String _payloadToCsv(UserDataCategory category, dynamic payload) {
    ...
    if (payload is List) {
      ...
      if (payload.first is Map) {
        ...
        for (final row in payload) {
          rows.add(
            header.map((k) => (row as Map)[k]).toList(),
          );
        }
      } else {
  ...
  void _appendHtmlSection(StringBuffer buffer, dynamic payload) {
    ...
    if (payload is List) {
      ...
      if (payload.first is Map) {
        ...
        buffer
          ..writeln('<ul>')
          ..writeAll(payload.map((e) => '<li>${_htmlEscape(e.toString())}</li>'),
              '\n')
          ..writeln('\n</ul>');
      }
    } else if (payload is Map) {
  ...
  Future<dynamic> _payloadFor(
    ...
    switch (category) {
      case UserDataCategory.wifiScanHistory:
        final snapshots =
            await _wifiHistory.loadSnapshots(limit: 10000);
        return snapshots
      ...
      case UserDataCategory.securityEvents:
        ...
        return events.map((e) => _eventToJson(e, anonymize: anonymize)).toList();
      ...
      case UserDataCategory.pinnedNetworks:
        return _favorites.pinned
            .map((b) => _maskBssid(b, anonymize))
            .toList();

      case UserDataCategory.scoreHistory:
        final scores =
            await _scoreHistory.getRecentScores(limit: 10000);
        return scores
            .map((s) => {
                  'score': s.score,
  ...
  Map<String, dynamic> _readPrefixed(
    String prefix, {
    required bool anonymize,
  }) {
    ...
    required bool anonymize,
  }) =>
      {
        ...
        'networks': snapshot.networks
            ...
            .toList(),
        'channel_stats': snapshot.channelStats
            .map((c) => {
                  'channel': c.channel,
  ...
  Map<String, dynamic> _heatmapSessionToJson(
    ...
    required bool anonymize,
  }) =>
      {
        ...
        'points':
            session.points.map((p) => _heatmapPointToJson(p, anonymize: anonymize)).toList(),
      };
  ...
  Map<String, dynamic> _lanSessionToJson(
    ...
    required bool anonymize,
  }) =>
      {
        ...
        'hosts': session.hosts
            .map((h) => _hostToJson(h, anonymize: anonymize))
            .toList(),
      };
  ...
  Map<String, dynamic> _hostToJson(
    ...
    required bool anonymize,
  }) =>
      {
        ...
        'exposure_findings': host.exposureFindings
